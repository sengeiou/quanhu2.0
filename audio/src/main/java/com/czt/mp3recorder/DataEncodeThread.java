...
public class DataEncodeThread extends HandlerThread implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    public void sendStopMessage(boolean delete) {
        check();
        Message msg = mHandler.obtainMessage();
        msg.what = PROCESS_STOP;
        msg.obj = delete;
        msg.sendToTarget();
//        mHandler.sendEmptyMessage(PROCESS_STOP);
    ...
    private int processData() {
        Log.d("yeying", "processData mTasks.size()" + mTasks.size());
        if (mTasks.size() > 0) {
            Task task = mTasks.remove(0);
            short[] buffer = task.getData();
            int readSize = task.getReadSize();
            Log.d("yeying", "processData readSize" + readSize);
            int encodedSize = LameUtil.encode(buffer, buffer, readSize, mMp3Buffer);
            Log.d("yeying", "processData encodedSize" + encodedSize);
            if (encodedSize > 0) {
                try {
