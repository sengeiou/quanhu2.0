...
var JsSdk = function () {
    function JsSdk() {
        ...
        this.baseUrl = '';
        this.apis = { 'config': { method: 'get' } };
        this.appInfo = {};
    }

    _createClass(JsSdk, [{
        key: 'send2Native',
        value: function send2Native(invokeName, data) {
            //只有通过校验才能发起原生调用
            //if(!this.isVerification && invokeName!=='config'){
            //    return;
            ...
            this.callbacks[invokeId.toString()] = data;
            var invokeData = JSON.stringify(message);
        ...
        key: 'receiveData',
        value: function receiveData(rawData, callback) {

            var data = JSON.parse(rawData);
            var index = data.errMsg.indexOf(':');
            var invokeResult = data.errMsg.substring(index + 1);
            if (!callback) {
                var invokeId = data.invokeId;
                callback = this.callbacks[invokeId.toString()];
            }

            if (!callback) return;
            var cb = callback[invokeResult];
            cb && cb(data, rawData);
        }
    }, {
        key: 'registerNative',
        value: function registerNative(invokeName) {
            var _this = this;

            return function (data) {
                _this.send2Native(invokeName, data);
            };
        }
    }, {
        key: 'registerApi',
        value: function registerApi(apiName) {
            var _this2 = this;

            return function (data) {
                _this2.send2Api(apiName, data);
            };
        }
    }, {
        key: 'send2Api',
        value: function send2Api(apiName, data) {
            var _this3 = this;

            var api = this.apis[apiName];
            var url = this.baseUrl + '{api.url}';
            var body = Object.assign({}, this.appInfo, data);
            fetch(url, { method: api.method, body: body }).then(function (json) {
                json = JSON.parse(json);
                _this3.receiveData(json, data);
            });
        }
    }]);
...
var sdk = new JsSdk();
var yryz = {
    'alert': sdk.registerNative('alter'),
    'config': sdk.registerApi('config'),
    '_send2Web': sdk.receiveData.bind(sdk),
    ...
    openYrSquare: sdk.registerNative('openYrSquare'),
