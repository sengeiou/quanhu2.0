...
public class CharacterParser {
    ...
    public static String[] pystr = new String[]{"a", "ai", "an", "ang", "ao",
            ...
            "dang", "NewsDao", "de", "deng", "di", "dian", "diao", "die", "ding",
    ...
    public String getSelling(String chs) {
        ...
        for (int i = 0; i < chs.length(); i++) {
            ...
            if (key.getBytes().length >= 2) {
                value = Pinyin.toPinyin(key.charAt(0));
                if (value == null) {
                    value = "unknown";
                }
            } else {
    ...
    /**
     * 词组解析，获取汉字拼音首字母简拼 * * @param chs * @return
     */
    public String getFirstSelling(String chs) {
        ...
        for (int i = 0; i < chs.length(); i++) {
            key = chs.substring(i, i + 1);
            if (key.getBytes().length >= 2) {
                value = (Pinyin.toPinyin(key.charAt(0))).substring(0, 1);
                if (value == null) {
                    value = "unknown";
                }
            } else {
     ...
     * @return
     */
    public boolean hasCH(String str) {
