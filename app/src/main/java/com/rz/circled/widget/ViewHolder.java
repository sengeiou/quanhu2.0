...
public class ViewHolder {
    ...
    public ViewHolder setImageByUrlTransger(ImageView img, String url, int loadImg) {
        if (mContext instanceof Activity) {
            Activity act = (Activity) mContext;
            if (act == null || act.isFinishing()) {
                return this;
        ...
        if (Protect.checkLoadImageStatus(mContext)) {
            Glide.with(mContext).load(url).placeholder(loadImg).centerCrop().error(loadImg).into(img);
        }
        return this;
    }

    public ViewHolder setImageByUrl(ImageView img, String url) {
        if (mContext instanceof Activity) {
            Activity act = (Activity) mContext;
            if (act == null || act.isFinishing()) {
                return this;
