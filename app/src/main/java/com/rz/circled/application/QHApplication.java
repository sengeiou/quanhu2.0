...
import com.rz.circled.js.KickOutHandler;
...
import com.rz.circled.ui.activity.LoadResActivity;
...
public class QHApplication extends BaseApplication {
    ...
    public void onCreate() {
        ...
        instance = this;
        configYunXin();
        AdvancedWebView.cacheAble = !BuildConfig.isDebug;
...
//        configRecord();
    ...
    private void registerJsServerInterface() {
        ...
        //被踢下线
        RegisterList.registerServerHandlerClass(KickOutHandler.class);
...
//                    requestBuilder.header("devId", SystemUtils.getIMEI(mContent));
    ...
    protected void attachBaseContext(Context base) {
        ...
        if (!quickStart() && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {//>=5.0的系统默认对dex进行oat优化
            boolean needWait = needWait(base);
            if (needWait) {
                waitForDexopt(base);
            }
