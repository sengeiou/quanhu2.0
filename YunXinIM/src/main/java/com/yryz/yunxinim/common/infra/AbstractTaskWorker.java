...
public abstract class AbstractTaskWorker {	
	...
	private final Executor getExecutor(Task task) {
		...
		return com.yryz.yunxinim.common.infra.TaskExecutor.IMMEDIATE_EXECUTOR;
	}
	
	private final Runnable getRunnable(final com.yryz.yunxinim.common.infra.Task task) {
		return new Runnable() {
			@Override
			public void run() {
				// schedule
