...
public class TaskManagerRegistry {
    ...
    public static void waitAll(final Context context, final Runnable done, final int max, final int interval) {
		...
		com.yryz.yunxinim.common.infra.Handlers.sharedHandler(context).postDelayed(new Runnable() {
			int count;

			@Override
			public void run() {
				if (!idle() && count++ < max) {
					com.yryz.yunxinim.common.infra.Handlers.sharedHandler(context).postDelayed(this, interval);
				} else {
					done.run();
				}
			}
