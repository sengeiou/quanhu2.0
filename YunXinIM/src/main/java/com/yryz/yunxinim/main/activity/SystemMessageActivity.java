...
public class SystemMessageActivity extends UI implements TAdapterDelegate,
    ...
    private static final boolean MERGE_JOIN_TEAM_VERIFY = true; // 是否要合并加群申请，同一个用户仅保留最近一条申请内容（默认不合并）
    ...
    public static void start(Context context, boolean group) {
    ...
    private Set<Map<String, String>> joinTeamVerifyRequestAccounts = new HashSet<>(); // 发送过加群申请的账号（加群申请合并用）
    ...
    public void loadMessages() {
        ...
        while (true) {
            ...
            for (SystemMessage m : temps) {
                if (m.getType() == SystemMessageType.AddFriend)
                    continue;
                ...
                // 同一个账号的加群申请仅保留最近一条
                if (applyJoinTeamVerifyFilter(m)) {
                    continue;
                }
    ...
    private void onIncomingMessage(final SystemMessage message) {
        ...
        // 同一个账号的加群申请仅保留最近一条
        if (applyJoinTeamVerifyFilter(message)) {
            SystemMessage del = null;
            for (SystemMessage m : items) {
                if (m.getFromAccount().equals(message.getFromAccount()) && m.getTargetId().equals(message.getTargetId()) && m.getType() == SystemMessageType.ApplyJoinTeam) {
                    del = m;
                    break;
                }
            }
    ...
    // 同一个账号的好友申请仅保留最近一条
    private boolean applyJoinTeamVerifyFilter(final SystemMessage msg) {
        if (!MERGE_JOIN_TEAM_VERIFY) {
            return false; // 不需要MERGE，不过滤
        }
        ...
        for (Map<String, String> item : joinTeamVerifyRequestAccounts) {
            if (TextUtils.equals(item.get(msg.getFromAccount()), msg.getTargetId())) {
                return true; // 过滤
            }
        }

        Map<String, String> item = new HashMap<>();
        item.put(msg.getFromAccount(), msg.getTargetId());
        joinTeamVerifyRequestAccounts.add(item);
        return false; // 不过滤
    }
