...
public class WatchVideoActivity extends UI implements Callback {
    ...
    private void showVideoInfo() {
        ...
        if (duration <= 0) {
            fileInfoTextView.setText(getString(R.string.file_size) + FileUtil.formatFileSize(fileSize));
        } else {
            long seconds = TimeUtil.getSecondsByMilliseconds(duration);
            fileInfoTextView.setText(getString(R.string.file_size) + FileUtil.formatFileSize(fileSize) + getString(R.string.default_time1)
                    + String.valueOf(seconds) + getString(R.string.default_sec1));
            videoLength = seconds;
        }
    }
