...
import com.yryz.yunxinim.uikit.recent.viewholder.CommonRecentViewHolder;
import com.yryz.yunxinim.uikit.recent.viewholder.RecentContactAdapter;
import com.yryz.yunxinim.uikit.recent.viewholder.RecentViewHolder;
import com.yryz.yunxinim.uikit.recent.viewholder.TeamRecentViewHolder;
...
public class RecentContactsFragment extends TFragment implements TAdapterDelegate {
    ...
    private void showLongClickMenu(final RecentContact recent) {
        ...
        alertDialog.setTitleVisible(true);
        String title = getString(R.string.main_msg_list_delete_chatting);
        alertDialog.addItem(title, R.color.color_main, new onSeparateItemClickListener() {
            @Override
            public void onClick() {
                // 删除会话，删除后，消息历史被一起删除
...
//                }
////                Team team = NIMClient.getService(TeamService.class).queryTeamBlock(r.getContactId());
////                if (team != null && !team.isMyTeam()) {
////                    NIMClient.getService(MsgService.class).deleteRecentContact(r);
////                    NIMClient.getService(MsgService.class).clearChattingHistory(r.getContactId(), r.getSessionType());
////                    items.remove(r);
////                }
//            }
        }
    ...
    private void onRecentContactChanged(List<RecentContact> recentContacts) {
        ...
        for (RecentContact r : recentContacts) {
            ...
            if (index >= 0) {
                if (!NimUIKit.getMessageFilterListener().messageFilter(r.getAttachment())) {
                    RecentContact contact = items.get(index);
                    if (contact.getExtension() != null
                            && contact.getExtension().containsKey(Extras.EXTRA_TYPE)
                            && (boolean) contact.getExtension().get(Extras.EXTRA_TYPE)
            ...
            String msgId = r.getRecentMessageId();
            ...
            if (msgs != null && !msgs.isEmpty()) {
                ...
                if ((content != null
                        && !content.isEmpty()
                        && content.containsKey(Extras.EXTRA_AT_LIST)
                        && ((List<String>) content.get(Extras.EXTRA_AT_LIST)).contains(NimUIKit.getAccount()))
                        || isAt) {
                    Map<String, Object> map = new HashMap<>();
                    map.put(Extras.EXTRA_TYPE, r.getUnreadCount() > 0);
                    r.setExtension(map);
                }
            }
    ...
    TeamDataCache.TeamDataChangedObserver teamDataChangedObserver = new TeamDataCache.TeamDataChangedObserver() {
        ...
        public void onRemoveTeam(Team team) {
            for (RecentContact recent : items) {
                if (TextUtils.equals(recent.getContactId(), team.getId())) {
                    // 删除会话，删除后，消息历史被一起删除
                    NIMClient.getService(MsgService.class).deleteRecentContact(recent);
                    NIMClient.getService(MsgService.class).clearChattingHistory(recent.getContactId(), recent.getSessionType());
                    items.remove(recent);
